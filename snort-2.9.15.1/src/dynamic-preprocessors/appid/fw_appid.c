...
{
    if (session->tpPayloadAppId == APP_ID_HTTP_TUNNEL && !getAppIdFlag(session, APPID_SESSION_HTTP_TUNNEL))
    {
        if (app_id_debug_session_flag)
...
static inline void ExamineSslMetadata(SFSnortPacket *p, APPID_SESSION_DIRECTION direction, tAppIdData *session, tAppIdConfig *pConfig)
{
    ...
    if ((session->scan_flags & SCAN_SSL_CERTIFICATE_FLAG) && session->tsession->tls_cname)
    {
