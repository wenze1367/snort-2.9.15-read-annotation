...
static void Frag3Defrag(Packet *p, void *context)
{
    ...
    if (ft == NULL)
    {
         ...
         *根据传入的IP分片数据包，创建新的frag tracker节点，
		 *初始化该节点的二级splay树，调用InsertFrag把当前分片插
		 *入该二级splay树，再调用ubi—sptInsert把新节点插入到主
		 *splay树。 重要的是创建新frag tracker节点的数据包必须是
		 *第一个分片数据包。
         *
         *
         *
     ...
     *判断ft节点对应的二级splay树中存储的分片数据包是否收集完整.
     */
 ...
 *
 *
(1)创建fragtracker节点并填充数据；初始化一个用于存储分片IP包的二级splay树：
(2)把当前分片数据包插入刚创建的二级二叉树；
(3)把创建的fragtracker节点插入主spaly树用于对IP分片数据包进行跟踪。
 */
static int Frag3NewTracker(Packet *p, FRAGKEY *fkey, Frag3Context *f3context)
{
    ...
    // Try to get a new one,创建新frag tracker节点,
...
*
 *(1)该函数用于把当前IP分片数据包插入到二级spalyat中，存储该数据包。
在插入的过程中，先要判断该分片是否已经．被存储，如果是新的分片则执行
插入操作。
 */
        ...
        Frag3Context *f3context)
{
     ...
     *
     *因新接收的新分片数据包，而更新该数据包对应主splay树的跟踪数据
包的 FragTracker节点,更新上一个分片出现的时间。
     */
