...
int ConsumeSSFileCache(const uint8_t *buf, uint32_t len)
{
    ...
    if( !buf )
    {
        FILE_ERROR("Side channel: No buffer");
        return -1;
    }

    if( len < sizeof(*hk) + sizeof(*hv) )
    {
        FILE_ERROR("Side channel: length too small");
        return -1;
    }
    hk = (FileHashKey *)buf;
    ...
    else if (sfxhash_add(fileHash, hk, hv) != SFXHASH_OK)
    {
       ...
       * gracefully.
       */
      FILE_ERROR("Side channel: Failed to add file node to hash table");
      pthread_mutex_unlock(&file_cache_mutex);
      return -1;
    }
    pthread_mutex_unlock(&file_cache_mutex);
  ...
  file_sha256_print(hv->sha256);
#endif
   FILE_DEBUG("Side channel: Consume verdict: %d file id: %d",hv->verdict,hv->file_type_id); 
